function  [d,mu,lam,val,k]=SQPsub(dfk,Bk,Ae,hk,Ai,gk)
     % 求解二次规划子问题:  min   qk(d)=0.5*d'*Bk*d+dfk'*d,
     ...
     %参数选取：
     gamma=0.05; epsilon=1.0e-2; rho=0.5;sigma=0.5;ep0=0.5;
     n=length(dfk);  l=length(hk); m=length(gk);  
     mu0=0.05*zeros(l,1); lam0=0.05*zeros(m,1);d0=ones(n,1);u0=[ep0;zeros(n+l+m,1)];
     z0=[ep0; d0; mu0;lam0,];
     k=0;   %k为迭代次数
     ...
     %%%%%%%%%%%%%%%%%%%%%%%%%% 
     function [dd1,dd2,v1]=ddv(ep,d,lam,Ai,gk) %%%求vi
     m=length(gk);
